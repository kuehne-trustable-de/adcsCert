 ...
 */
	...
	public KeyCertBundle newKeyBundle(String bundleName) throws GeneralSecurityException {
		...
		try {
		    ...
		    String csr = createCsrAsPEM(hostname,
		            kp.getPublic(), 
		            kp.getPrivate() );
			...
			if( SubmitStatus.ISSUED.equals(certResponse.getStatus())) {
				X509Certificate cert = CryptoUtil.convertPemToCertificate(certResponse.getB64Cert());
				X509Certificate caCert = CryptoUtil.convertPemToCertificate(certResponse.getB64CACert());
				X509Certificate[] caCertArr = {cert, caCert};
				
				LOG.debug("succeeded to retrieve certificate from ADCS instance");
				return new KeyCertBundle( bundleName, caCertArr, cert, kp.getPrivate());

			} else {
				LOG.error("failed to retrieve certificate from ADCS instance, status : '{}', check request id {}", certResponse.getStatus(), certResponse.getReqId());
	 ...
	 * @throws IOException
	 */
    private String createCsrAsPEM(final String hostname,
            PublicKey pubKey, 
            PrivateKey priKey )
        ...
        JcaContentSignerBuilder signerBuilder = new JcaContentSignerBuilder(signingAlgo);
        ...
        namesList.add(new GeneralName(GeneralName.dNSName, hostname.toLowerCase()));
